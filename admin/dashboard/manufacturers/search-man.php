...
if(isset($_REQUEST["term"]) && strlen($_REQUEST['term']) >= 3){
    $var = $_REQUEST["term"];
    $vyr = "SELECT * FROM vyrobcovia WHERE v_nazov LIKE ? LIMIT 20";
          ...
          if(mysqli_num_rows($result) > 0){
              ...
              while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
                   echo '<div class="category card " style="border: 1px solid #E7E7E7; padding: 10px; margin-bottom: 10px; background-color: white; box-shadow: 2px 2px 10px -2px #616161; border-radius: 20px;">';
                    ...
                    echo '<span style="padding-left: 10px;"><b>'.$row['v_nazov'].'</b></span>
                    <form method="post" action="index.php" >
                    
                    <button type="submit" name="set" value="'.$row['v_id'].'" class="btn" style="all:unset; cursor: pointer;"><i style="font-size: 20px" class="fas fa-cog"></i></button>
                    </form>
                </div>';
                echo '</div>';
            }
